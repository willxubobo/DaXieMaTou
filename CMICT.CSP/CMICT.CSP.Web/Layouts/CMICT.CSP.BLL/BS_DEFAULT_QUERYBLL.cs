...
namespace CMICT.CSP.BLL
{
	...
	public partial class BS_DEFAULT_QUERYBLL
	{
		...
		public bool Add(CMICT.CSP.Model.BS_DEFAULT_QUERY model)
		{
			...
			strSql.Append("ID,TemplateID,ColumnName,Desction,CompareValue,Compare,Created,Modified,Author,Editor)");
			...
			parameters[3].Value = model.Desction;
			parameters[4].Value = model.CompareValue;
		...
		public CMICT.CSP.Model.BS_DEFAULT_QUERY GetModel(Guid ID)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,TemplateID,ColumnName,Desction,CompareValue,Compare,Created,Modified,Author,Editor from BS_DEFAULT_QUERY ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
                                            new SqlParameter("@ID", SqlDbType.UniqueIdentifier,16)
			};
