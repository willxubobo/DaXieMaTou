...
namespace CMICT.CSP.BLL
{
	...
	public partial class BS_CUSTOM_QUERYBLL
	{
		...
		public bool Add(CMICT.CSP.Model.BS_CUSTOM_QUERY model)
		{
			...
			strSql.Append("ID,TemplateID,ColumnName,DisplayName,ControlType,Compare,Sequence,Reminder,Created,Modified,Author,Editor)");
			...
			parameters[7].Value = model.Reminder;
		...
		public CMICT.CSP.Model.BS_CUSTOM_QUERY GetModel(Guid ID)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,TemplateID,ColumnName,DisplayName,ControlType,Compare,Sequence,Reminder,Created,Modified,Author,Editor from BS_CUSTOM_QUERY ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
                                            new SqlParameter("@ID", SqlDbType.UniqueIdentifier,16)
			};
