...
namespace CMICT.CSP.BLL
{
	...
	public partial class BS_COMPUTEBLL
	{
		...
		public bool Add(CMICT.CSP.Model.BS_COMPUTE model)
		{
            ...
            strSql.Append("ID,TemplateID,ComputeColumn,DisplayName,MergeColumnName,Sequence,DecimalCount,Created,Modified,Author,Editor)");
            strSql.Append(" values (");
            strSql.Append("@ID,@TemplateID,@ComputeColumn,@DisplayName,@MergeColumnName,@Sequence,@DecimalCount,@Created,@Modified,@Author,@Editor)");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.UniqueIdentifier,16),
					new SqlParameter("@TemplateID", SqlDbType.UniqueIdentifier,16),
					new SqlParameter("@ComputeColumn", SqlDbType.NVarChar,1000),
					...
					new SqlParameter("@DecimalCount", SqlDbType.Int,4),
			...
			parameters[2].Value = model.ComputeColumn;
            ...
            parameters[6].Value = model.DecimalCount;
		...
		public bool Update(CMICT.CSP.Model.BS_COMPUTE model)
		{
            ...
            strSql.Append("DecimalCount=@DecimalCount,");
		...
		public CMICT.CSP.Model.BS_COMPUTE GetModel(Guid ID)
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 * from BS_COMPUTE ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
                                            new SqlParameter("@ID", SqlDbType.UniqueIdentifier,16)
			};
		...
		public CMICT.CSP.Model.BS_COMPUTE DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
                ...
                if (row["DecimalCount"] != null && row["DecimalCount"].ToString() != "")
                {
                    model.DecimalCount = int.Parse(row["DecimalCount"].ToString());
                }
