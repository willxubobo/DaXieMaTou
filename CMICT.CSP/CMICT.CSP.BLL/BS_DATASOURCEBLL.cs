...
namespace CMICT.CSP.BLL
{
	...
	public partial class BS_DATASOURCEBLL
	{
        ...
        /// 判断数据源名称是否存在
        /// </summary>
        public bool GetExists(string sourcename,string sourceid)
        {
            //该表无主键信息，请自定义主键/条件字段
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from BS_DATASOURCE ");
            strSql.Append("  where SourceName = @SourceName");
            ...
            if (rows!=null&&rows.ToString()!=""&&Convert.ToInt32(rows)>0)
            {
                return true;
            }
            else
